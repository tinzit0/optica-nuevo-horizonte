...
async function cargarProductosConFiltros() {
    ...
    grid.innerHTML = visibles.map(p => {
        ...
        const accion = tienePrecio ? `<button onclick="agregarAlCarritoDirecto('${p.id}', '${p.title}', '${p.brand}', ${p.price}, '${p.image}')" class="btn-gold">AÑADIR A LA BOLSA</button>` : `<a class="btn-gold availability-link" href="producto.html?id=${encodeURIComponent(p.id)}">CONSULTAR EN TIENDA</a>`;
