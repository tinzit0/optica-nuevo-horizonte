-- Etapa 3.3 B: integra el código estable del catálogo con los UUID internos.
-- Conserva la firma y toda la lógica de create_optica_order() definida en 006.
...
create unique index if not exists optica_productos_product_code_unique
  on public.optica_productos (product_code);
...
begin
  ...
  loop
    ...
    select p.title, p.brand, p.price, p.stock, p.published
      ...
      where (p.product_code = product_id_text
         or p.id::text = product_id_text)
      limit 1;
